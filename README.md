...
- **A toolbar that goes where you want it** — a floating bubble you can drag to any edge; it turns into a column on the sides. When it stops fitting it hands whole sections to a ⋯ menu, named, rather than shrinking what is left — everything stays reachable, some of it one click further away. Every panel on the board keeps clear of whichever edge it is on
